...
@Component({
  ...
  changeDetection: ChangeDetectionStrategy.OnPush, // Added OnPush strategy for performance
  animations: [
...
export class SocialViewComponent implements OnInit, OnDestroy {
  ...
  addComment(shopping: Center, marketSurveyId: number): void {
    ...
    this.PlacesService.GenericAPI(body).subscribe({
      next: (response: any) => {
        if (response?.json?.id) {
          const index = shopping.ShoppingCenter.Comments.findIndex(
      ...
      error: () => {
        shopping.ShoppingCenter.Comments =
          shopping.ShoppingCenter.Comments.filter(
            (c) => c.Id !== newComment.Id
          );
        this.cdr.markForCheck();
      },
  ...
  addReply(marketSurveyId: number, commentId: number): void {
    ...
    if (shoppingCenter) {
      ...
      this.PlacesService.GenericAPI(body).subscribe({
        next: (response: any) => {
          if (response?.json?.id) {
            const index = shoppingCenter.ShoppingCenter.Comments.findIndex(
              (c) => c.Id === newReply.Id
            );
            if (index !== -1) {
  ...
  isDisliked(shopping: Center): boolean {
    ...
    return shopping.ShoppingCenter.Reactions.some(
      (reaction) =>
        reaction.ReactionId === 2 && reaction.ContactId === contactId
    );
  }
  addDislike(shopping: Center, reactionId: number): void {
    ...
    if (this.isLiked(shopping)) {
      shopping.ShoppingCenter.Reactions =
        shopping.ShoppingCenter.Reactions.filter(
          (reaction) =>
            !(reaction.ReactionId === 1 && reaction.ContactId === contactId)
        );
  ...
  trackByReplyId(index: number, item: any): number {
    return item.Id || index;
  }
  ...
  getVisibleCommentsCount(
    ...
    if (!comments) return 0;
    const sortedComments = this.sortCommentsByDate(
      comments.filter((c) => !c.ParentCommentId)
    );
    const visibleParentComments = this.showAllComments[marketSurveyId]
      ? sortedComments.length
      : Math.min(sortedComments.length, 3);
    ...
    sortedComments.slice(0, visibleParentComments).forEach((comment) => {
      ...
      visibleRepliesCount += this.showAllReplies[comment.Id]
        ? replies.length
        : Math.min(replies.length, 2);
    });
  ...
  shouldShowMoreComments(shopping: any): boolean {
    ...
    const totalCount = this.getTotalCommentsCount(
      shopping.ShoppingCenter.Comments
    );
    const visibleCount = this.getVisibleCommentsCount(
