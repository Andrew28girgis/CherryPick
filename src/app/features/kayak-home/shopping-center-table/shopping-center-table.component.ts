...
export class ShoppingCenterTableComponent implements OnInit, OnDestroy {
  ...
  private syncFilterState(): void {
    ...
    const serviceStageId = this.shoppingCenterService.getCurrentSelectedStageId()
  ...
  groupTenantsByAlphabet(): void {
    const sortedTenants = [...this.filteredTenants].sort((a, b) => a.Name.localeCompare(b.Name))
  ...
  goToTenant(tenant: Tenant) {
    this.router.navigate(["/dashboard", tenant.Id, tenant.OrganizationId, tenant.Name, tenant.Campaigns[0].Id])
  }
  ...
  filterDropdownOptions(): void {
    if (window.innerWidth < 768) {
      this.dropdowmOptions = this.dropdowmOptions.filter((option) => option.id !== 2)
    }
  }
