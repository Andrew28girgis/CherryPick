...
import  { ICampaign, Stage, Submission } from "src/app/shared/models/icampaign"
import  { EmilyService } from "src/app/core/services/emily.service"
import  { Router } from "@angular/router"

@Component({
  selector: "app-campaign-manager-card-view",
  templateUrl: "./campaign-manager-card-view.component.html",
  styleUrls: ["./campaign-manager-card-view.component.css"],
})
export class CampaignManagerCardViewComponent implements OnInit {
  ...
  toggleView(mode: "table" | "card"): void {
    this.viewMode = mode
    // You could save this preference in localStorage if you want it to persist
  ...
  ngOnInit(): void {
    ...
    if (savedViewMode) {
      ...
      this.viewMode = window.innerWidth <= 767 ? "card" : "table"
    }
  }
  ...
  onSearchCampaign(): void {
    if (this.searchCampaign && this.searchCampaign.trim().length > 0) {
      this.filteredCampaigns = this.campaigns.filter((c) =>
        c.Campaigns.some((campaign) => campaign.CampaignName.toLowerCase().includes(this.searchCampaign.toLowerCase())),
      )
    } else {
  ...
  getStageOrganizationCount(stageName: string, stages?: Stage[]): number | null {
    if (stages) {
      return stages.find((stage) => stage.stageName.toLowerCase() == stageName.toLowerCase())?.Organizations || null
    }
    return null
  }
  ...
  sortedSubmissions(submissions: Submission[]) {
    return submissions?.sort((a, b) => this.statusSortOrder(a.StatusId) - this.statusSortOrder(b.StatusId)) || []
  }
